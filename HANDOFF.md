...
2026-09-26T09:05Z | remote | DONE  | claude/remote-station-tab | see PR #38 -- config/agents.yaml (Raqib + 3 StarNet-derived roles, MIT-credited), code/station/ (roster, store, one worker per agent, 5 routes behind require_token), Station tab in web/, docs/STATION.md. 64 pytest tests in CI (deps pinned to venv-ai), 28/28 mutations caught, built UI driven in headless Chromium against the real package with a stand-in model. app.py untouched.
2026-09-26T09:05Z | remote | ASK   | claude/remote-station-tab | FOR LOCAL (app.py is yours), one step once #38 is on master: add `from code.station import mount_station` and `station = mount_station(app, require_token, Path(__file__).parent)` anywhere after require_token is defined -- placement relative to the SPA fallback does not matter, mount_station moves its routes to the front itself (tested against a catch-all registered first). Then `cd web && npm run build`, restart hermes-api, and `curl -s http://127.0.0.1:8000/api/station/agents`. A roster mistake fails startup with the agent and field named in logs/supervisord/hermes-api.err.log.
