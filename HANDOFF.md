...
2026-09-25T16:30Z | remote | CLAIM | claude/remote-starnet-gateway | Handoff Priority 2, route StarNet through Jarvis, at Ahmed's request ("A then C, go ahead"). NEW code/openai-gateway.js + code/test-openai-gateway.js + its CI line + docs/STARNET_GATEWAY.md. A SECOND OpenAI-compatible endpoint on 127.0.0.1:8001 that speaks StarNet's wire (SSE stream with a [DONE] sentinel, full message history, tool calls passed through) over registry.js's tiers. app.py and its /v1 routes are UNTOUCHED. registry.js gains one export (bumpQuota), nothing else.
2026-09-25T16:50Z | remote | DONE  | claude/remote-starnet-gateway | see PR #37 -- code/openai-gateway.js on 127.0.0.1:8001 + code/test-openai-gateway.js (54 assertions, 28/29 mutations; the escape is equivalent), in CI. 44 suites, 1018 assertions; sweep and weekly sweep clean, oxlint clean. Contract checked by running StarNet's REAL adapter against it: text, parallel tool calls, finish_reason and usage all arrive, and a reply slower than StarNet's 30s connect ceiling still comes through. app.py untouched. registry.js: one new export (bumpQuota), so both paths count one quota.
2026-09-25T16:50Z | remote | NOTE  | claude/remote-starnet-gateway | FOR LOCAL: app.py's /v1 comment says 'OpenAI-compatible adapter for StarNet'. It is not, and StarNet's own adapter shows why: against its single JSON body StarNet yields ZERO text events (the run ends 'done, truncated' with nothing in it). It still suits Aider/Continue, so I did not ask for a change -- only for the StarNet line in that comment to go when you next touch the file. The handoff's StarNet launch command pointing at :8000 is superseded by :8001 (docs/STARNET_GATEWAY.md).
2026-09-25T16:50Z | remote | ASK   | claude/remote-starnet-gateway | FOR AHMED, about 5 minutes on the Chromebook, which is the only proof CI cannot give: run the three curl commands in docs/STARNET_GATEWAY.md, then relaunch StarNet with CUSTOM_OPENAI_BASE_URL=http://127.0.0.1:8001/v1, type the base URL WITH http:// in Settings, LEAVE THE KEY EMPTY, and set Raqib to CUSTOM -> fast. Tell me what /tmp/jx-gateway.log says. Supervisord autostart is NOT added: the block is in the doc, and adding it is your call.
2026-09-26T08:40Z | remote | NOTE  | claude/remote-starnet-gateway | CORRECTION to my own rows above: the gateway port is 8010, NOT 8001. 8001 is tts-worker's (app.py TTS_WORKER_URL, code/tts_worker.py), so the first version would have failed to start on the Chromebook, and StarNet pointed at it would have reached the voice worker. Caught before merge by reading app.py; now pinned by a test that reads every file binding a port. Every command in docs/STARNET_GATEWAY.md uses 8010.
2026-09-26T11:45Z | remote | CLAIM | claude/remote-starnet-gateway | Ahmed clarified he wants StarNet ITSELF, same exact interface, running on the Chromebook. Adding to this branch: scripts/starnet.sh (install/start/stop/status/logs/doctor/clear-lock) + its test + docs/STARNET_CHROMEOS.md. Runs upstream StarNet unmodified in web mode, wired to the gateway on :8010. Nothing else touched; app.py untouched.
2026-09-26T12:00Z | remote | DONE  | claude/remote-starnet-gateway | see PR #37 -- scripts/starnet.sh + code/test-starnet-launcher.js (17 assertions, 12/12 mutations) + docs/STARNET_CHROMEOS.md. StarNet itself ran end to end on Debian through the gateway: provider probe verified, chat and 87-tool task runs done at $0, kill switch surfaced in StarNet. Install fixes found by doing it: skip onnxruntime's CUDA download, prune 618 MB of GPU/Mac/Windows binaries, node-pty needs build-essential. Found and fixed my own bug: an unanchored pgrep made `stop` kill the terminal that ran it.
2026-09-26T12:00Z | remote | ASK   | claude/remote-starnet-gateway | FOR AHMED on the Chromebook: `sudo apt-get install -y build-essential python3`, then `scripts/starnet.sh doctor`, `scripts/starnet.sh install` (reuses your ~/starnet untouched), `scripts/starnet.sh start`, open http://127.0.0.1:8787. In StarNet choose ASK FOR APPROVAL, and Connect a brain -> CUSTOM -> http://127.0.0.1:8010/v1, key empty, model fast.
