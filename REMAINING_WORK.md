...
| File | Issue | Action |
|---|---|---|
| `README.md` | Describes `code/models.js` (deleted this session, was already dead before that) as the live routing layer; omits `web/`, `sentinel/`, `automation/phase-b/`, `app.py`, `hermes.py`, `tts_engine.py`, `stt_engine.py`, `packages/model-gateway`, `config/supervisord.conf` entirely — reads as a Week-1 snapshot of a system that's since grown a full FastAPI web app, PWA frontend, video pipeline, and voice stack. Also had the stale ~/.jarvis-x/STOP kill-switch path (written without backticks here because this is a report of a defect, not a claim of a path — see detector 4's self-reference limitation). | Rewritten in this session (Phase 6). **Kill-switch path verified correct 2026-09-26.** |
| `Design.md` (line 39) | Had the same stale ~/.jarvis-x/STOP kill-switch path reference, in an otherwise-current PWA design spec. | **DONE.** Verified 2026-09-26: line 39 now reads `.jarvis-x-STOP` (repo root). |
| `knowledge/Guidelines.md` | Had the same stale kill-switch path. Everything else in this file is accurate (verified `gemini.js`'s actual tier/model names match exactly). | **DONE — Ahmed fixed it.** Verified 2026-09-26: line 17 reads "if .jarvis-x-STOP exists in the repo root". This row asked him to correct it by hand because the file is Edit-denied to Claude Code; he did, and the row stayed open anyway. |
