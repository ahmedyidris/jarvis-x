...
Three verticals exist (`letters`, `economic_facts`, `commodities_macro`), each documented in `automation/phase-b/CONTEXT.md`'s "Adding a new vertical" checklist. No partially-built 4th/5th vertical found (checked `automation/phase-b/stages/01_source_content/output/` for any directory beyond the three, and `automation/phase-b/*.py` for any half-written generator). Built one more this session (Phase 2): **geopolitical-risk**, same economic-facts-shaped pattern (WebSearch-sourced, hardcoded, no live APIs).
