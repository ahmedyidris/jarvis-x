...
| Query resolution + response building + graceful degradation (Bitcoin / S&P 500 / Oil → data keys) | `test-agent-data-integration.js` **13/13 offline, in CI** as of 2026-09-07, plus 1 live case behind `JX_NET=1`. The previous "5/5" was never a measurement: the file ended in `.catch(console.error)` and exited 0 regardless, AND skipped entirely without `JX_NET`. Four of its five cases never needed the network. Five mutations now fail it. See REMAINING_WORK.md P0.8 |
| **Agent routing accuracy** | `eval-agent.js` **41/41 held-out = 100.0%** on Ahmed's Chromebook, 2026-09-07, `qwen2.5:3b`, three identical runs of 52 cases; mirror 8/8, tuned 3/3, gate (85% held-out) MET. Every category 100%. The number to cite is the four *blind* cases (`which files are sitting in bin`, `what has been put in the docker folder`, `is there anything in the archive folder`, `contents of bootstrap please`) — written after the prompt change, never run before, all passed. The 0.0 gap is NOT evidence: at held-out 100% it is ≤ 0 by arithmetic. Three identical runs is NOT sampling stability: `local.js` pins `temperature: 0`. See REMAINING_WORK.md P0.2 |
| Agent prompt construction | `test-agent.js` **15/15**. Before 2026-09-04 this file did not exist and `propose()` threw on every goal — `agent.js` was dead from commit `7720897` |
| **Self-diagnosis** (`code/selfdebug.js`) | `test-selfdebug.js` **30/30**, new 2026-09-07. Reads `logs/actions.jsonl`, groups failures, proposes nothing. Cannot write or execute — asserted at runtime with fs's writers stubbed to throw, and from the source. Its first live report is `0 failures counted, 599 UNATTRIBUTABLE`, because 599 of the log's failure rows are test fixtures and pre-v3 rows cannot be told apart. See REMAINING_WORK.md P0.6 |
| Audit-log provenance (`origin`, schema v3) | `test-guard.js` **21/21**. Every row records `test` or `app`, detected from the entry script. Without it a diagnostic reports "gemini 429 x29" about a string that exists only in `test-guard.js` |
| Scheduler | `test-scheduler.js` 8/8 |
| i18n EN/AR + ARIA labels | ⚠️ **NUMBER UNRELIABLE.** `test-accessibility.js` 3/3, `test-full-accessibility.js` 5/5 (after adding `i18next`) — but both files end in `.catch(console.error)` and exit 0 whatever happens, so these counts are what the files *printed*, not what they *verified*. Neither is in CI. Tallied in `docs/triage/2026-09-repo-triage.md` |
...
| Voice accent handling (Piper) | `test-voice.js` **3/3** on Ahmed's machine (en_US-amy, ar_JO-kareem, ar-AE-emirati-female all round-trip) — this one is a real result. ⚠️ `test-voice-accents.js` 4/4 and `test-voice-full-system.js` 9/9 are **UNRELIABLE**: both end in `.catch(console.error)` and exit 0 regardless |
