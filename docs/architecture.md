...
**Built but not wired in:** `packages/model-gateway` — a fully tested (47/47) standalone package for multi-provider routing/circuit-breaking/budget/telemetry. Its jarvis-x glue layer, gateway-adapter.js, is **not in this repo at all**: it was built on the unmerged `worktree-model-gateway` branch and deliberately excluded from the merge along with Tasks 9-11, because it was written against a long-forked snapshot of `guard.js` that predates master's. Corrected 2026-09-09 — this line previously named it as though it were present, and `code/weekly-sweep.js` flagged the dangling reference. Deliberately not connected to system 2 above. See `DECISION_RECORD_model-gateway.md` for the full reasoning — short version: it was built against an already-outdated snapshot of `guard.js`, and the real bugs it would have prevented were found and fixed directly and more simply instead.
