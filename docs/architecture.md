...
**A real gap, found and left as a decision for Ahmed, not silently "fixed":** `app.py`'s `/api/ask` endpoint does not itself check `STOP_FILE.exists()` before calling `hermes.py`/Ollama — only `/api/killswitch` reads/writes the flag. Setting the kill switch via the UI currently stops the JS agent-autonomy path (system 2) but does **not** stop a person from continuing to chat with Jarvis via the web UI (system 1). Whether that's intentional (chat isn't "autonomous action," so maybe it shouldn't be gated the same way) or a gap worth closing is a real product decision, not a bug fix — flagged in `REMAINING_WORK.md`, not changed.
