...
| Issue | Workaround |
|---|---|
| ~~`app.py`'s `/api/ask` doesn't check the kill switch~~ — **resolved 2026-08-16** | `/api/ask` now returns `503` when `.jarvis-x-STOP` exists, verified live (baseline works, switch blocks, clearing restores it). See `docs/architecture.md`'s kill-switch section for the decision rationale. |
